    ...
    <div className="flex flex-col items-center justify-center h-full">
      ...
      <div className="text-center mb-8 w-full">
        ...
        <div onClick={() => speakWord(word.finnish)} className="cursor-pointer group transition-transform active:scale-95">
          <div className="text-7xl mb-6 transform group-hover:scale-110 transition-transform duration-300">{word.emoji}</div>
          <div className="flex items-center justify-center gap-3 mb-2">
            <h2 className="text-5xl font-bold text-gray-800 dark:text-white">{word.finnish}</h2>
            <Volume2 className="w-8 h-8 text-blue-500 opacity-60 group-hover:opacity-100 transition-opacity" />
          </div>
        </div>

        {/* Translation - Always visible */}
        <p className="text-2xl text-gray-500 dark:text-gray-400 font-medium mt-2">{word.russian}</p>
      </div>

      {/* Example - Always visible if exists (Max 1) */}
      {word.example_sentence && (
        <div className="w-full bg-gray-50 dark:bg-gray-700/50 rounded-xl p-6 mb-8 border border-gray-100 dark:border-gray-700 text-center">
          <p className="text-sm text-gray-400 uppercase tracking-wider mb-3 font-semibold">Пример</p>
          <div className="flex items-center justify-center gap-4 text-left">
            ...
            <div>
              <p className="text-xl font-medium text-gray-800 dark:text-white mb-1 leading-snug">
                {word.example_sentence.finnish}
              </p>
              <p className="text-lg text-gray-500 dark:text-gray-400 leading-snug">
      ...
      {/* Controls */}
      <div className="w-full mt-auto pt-4 flex gap-4">
        <button
          onClick={() => {
        ...
        <button
          ...
          className={`flex-grow bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white text-lg font-bold py-4 rounded-xl shadow-lg hover:shadow-xl transition-all transform hover:-translate-y-0.5 active:translate-y-0 flex items-center justify-center gap-2`}
        >
          {currentWordIndex < words.length - 1 ? 'Дальше' : 'Далее'}
