...
export const checkGoogleTTSAvailability = async () => {
  ...
  try {
    ...
    console.log('🔊 Google TTS status:', googleTTSAvailable ? 'Available' : 'Not configured')
    return googleTTSAvailable
  } catch (error) {
...
export const speakWithGoogleTTS = async (text, language = 'fi-FI') => {
  try {
    ...
    // Если API вернул ошибку - используем fallback
    if (!response.ok || data.useFallback) {
      console.warn('⚠️ Google TTS unavailable, using Web Speech API')
      return speakWithWebSpeechAPI(text, language)
    }

    // Создаём Audio объект из base64
